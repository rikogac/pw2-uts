...
$product = $pdo->query("SELECT * FROM product WHERE id='$_GET[id]'")->fetchAll(PDO::FETCH_ASSOC)[0];
...
if (isset($_POST['edit'])) {
    ...
    $update = $pdo->prepare("UPDATE product SET sku=:sku, name=:name, purchase_price=:purchase_price, sell_price=:sell_price, stock=:stock, min_stock=:min_stock WHERE id='$_POST[id]'");
    $update->bindParam(":sku", $sku);
    $update->bindParam(":name", $name);
    $update->bindParam(":purchase_price", $purchase_price);
    $update->bindParam(":sell_price", $sell_price);
    $update->bindParam(":stock", $stock);
    $update->bindParam(":min_stock", $min_stock);
    $update->execute();
}
